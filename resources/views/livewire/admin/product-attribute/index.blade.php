<div>
    ...
    @include('admin.product-attribute.inc.deleteModal')
    <!-- Alert Message -->
    @include('flash-message')
    <!-- Page Title & Pagination -->
    @include('admin.product-attribute.inc.pageHeader')
    <!-- Search Product Attributes -->
    @include('admin.product-attribute.inc.searchFilter')
    ...
    <div class="card card-list">
        <div class="row">
                <div class="col-md-6">
                    <h5 class="card-header"><i class="bx bx-bar-chart-alt-2"></i> List of Product Attributes</h5>
                </div>
                <div class="col-md-6">
                    <div class="count-section">
                        <div class="record-div">
                            <span class="badge bg-label-primary record-label">Total Attributes</span>
        ...
        <div class="table-responsive text-nowrap">
                    ...
                    @if ($attributeList->count())
                            @foreach ($attributeList as $attribute)
                                <tr>
                                    ...
                                    <td>
                                        @if ($attribute->published == '1')
                                            <span class="badge bg-success badge-center"><i
                                                    class="bx bx-check bx-xs"></i></span>
                                        @else()
                                            <span class="badge bg-danger badge-center"><i class="bx bx-x bx-xs"></i></span>
                                        @endif
                                    </td>
                                    <td>{{ $attribute->displayOrder }}</td>
                                    <td class="d-flex justify-content-center">
                                        <div class="list-action-btn" data-bs-toggle="tooltip" data-bs-placement="left"
                                            title="Edit">
                                            <a href="{{ url('admin/attribute/edit/' . $attribute->id) }}"
            ...
            <div>
                {{ $attributeList->links('pagination-links') }}
            </div>
        </div>
    </div>
</div>
